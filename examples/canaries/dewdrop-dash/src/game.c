...
void swan_scene_render(swan_scene_id_t scene) {
    ...
    if (scene == SWAN_SCENE_TITLE) {
        ...
        swan_gfx_put_tile(0, (uint8_t)(9 + GAME_HOME_X),
                          (uint8_t)(5 + GAME_HOME_Y), play_tile(6));
        for (x = 0; x < GAME_GRID_WIDTH; ++x) {
            if (game_model_seed_at(x, 5, &mask) && (model.seeds & mask) == 0) {
                swan_gfx_put_tile(0, (uint8_t)(9 + x), 10, play_tile(4));
            }
        }
        swan_gfx_put_tile(0, (uint8_t)(9 + model.x),
                          (uint8_t)(5 + model.y), play_tile(3));
        for (index = 0; index < model.charge; ++index)
            swan_gfx_put_tile(0, (uint8_t)(2 + index), 2, play_tile(3));
